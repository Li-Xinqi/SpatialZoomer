...
def run_mcistgate(
    ...
    # clustering method -- MCIST_GATE
    n = 10
        
    adata = MCIST_GATE(adata = adata, n_clusters = n, spatial_rad_cutoff = 50, clustering_algo='Mclust')
    ...
    # save the results
    cluster_res_df.to_csv(save_path+data_name+f"_mcistgate_{n}.csv")
    pd.DataFrame(
        {'time_consumption(s)': [end_time - start_time]}
    ).to_csv(save_path+data_name+"_time_mcistgate.csv", index=False)
...
if __name__ == "__main__":
    ...
    ).to_csv(args.save_path + args.data_name+"_memory_mcistgate.csv", index=False)
