...
def run_mcistgraphst(
    ...
    # clustering method -- MCIST_GraphST
    n = 10
        
    adata = MCIST_GraphST(adata = adata, n_clusters = n, clustering_algo='Mclust')
    ...
    # save the results
    cluster_res_df.to_csv(save_path+data_name+f"_mcistgraphst_{n}.csv")
    pd.DataFrame(
        {'time_consumption(s)': [end_time - start_time]}
    ).to_csv(save_path+data_name+"_time_mcistgraphst.csv", index=False)
...
if __name__ == "__main__":
    ...
    ).to_csv(args.save_path + args.data_name+"_memory_mcistgraphst.csv", index=False)
